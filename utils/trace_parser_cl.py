...
from optc_parser import preprocess_optc_dataset
...
metadata = {
    ...
    'optc_1': {
        'train': ['0201_benign_2019-12-07T22-06-33.589.json', '0201_benign_2019-12-08T01-57-30.012.json', '0201_benign_2019-12-08T05-46-21.658.json', '0201_benign_last (1).json'],
        'test': ['SysClient0201.systemia.com.json']
    },
    'optc_2': {
        'train': ['0501_benign_2019-11-15T13-29-59.064.json', '0501_benign_2019-11-15T17-22-42.923.json', '0501_benign_last (1).json'],
        'test': ['SysClient0501.systemia.com.json']
    },
    'optc_3': {
        'train': ['0051_benign_2019-12-08T00-56-58.175.json', '0051_benign_2019-12-08T04-30-36.852.json', '0051_benign_last (1).json'],
...
def read_single_graph(dataset, malicious, path, test=False):
    ...
    path = './data_cl/{}/'.format(dataset) + path + '.txt'
    f = open(path, 'r')
    lines = []
    for l in f.readlines():
        ...
        # else:
            # print(ts)
            # ts = convert_to_unix_timestamp(ts.strip())
        if not test:
    ...
    node_uuid2cnt = {}
    for l in lines:
        src, dst, src_type, dst_type, edge_type, edge_time = l
        ...
        if dst not in node_map:
            ...
            node_uuid2cnt[dst] = node_cnt
            node_cnt += 1
        if not g.has_edge(node_map[src], node_map[dst]):
            g.add_edge(node_map[src], node_map[dst], type=edge_type_id, edge_time=edge_time)
    ...
    return node_map, g, node_uuid2cnt
...
def parse_node_name(line):
    ...
    if len(dstId1) > 0 and dstId1[0] != 'null':
        dst_id = dstId1[0]
        dst_name = dstId1name[0]
    else:
        dstId2 = pattern_dst2.findall(line)
        dstId2name = pattern_dst2_path.findall(line)
        if len(dstId2) > 0 and dstId2[0] != 'null':
            dst_id = dstId2[0]
            dst_name = dstId2name[0]

    src_uuid = pattern_src.findall(line)
    src_name = pattern_src_path.findall(line)
    if len(src_uuid) == 0 or len(src_name) == 0: return None, None, None, None
    srcId = src_uuid[0]
    srcIdname = src_name[0]

    return srcId, srcIdname, dst_id, dst_name
def preprocess_dataset(dataset):
    id_nodetype_map = {}
    id_nodename_map = {}
    for file in os.listdir('../MAGIC-main/data/{}/'.format(dataset)):
        if ('json' in file and 'ta1-' in file and not '.txt' in file and not 'names' in file and not 'delete' in file
                and not 'types' in file and not 'metadata' in file):
            print('reading {} ...'.format(file))
            f = open('../MAGIC-main/data/{}/'.format(dataset) + file, 'r', encoding='utf-8')
            for line in tqdm(f):
    ...
    for key in metadata[dataset]:
        for file in metadata[dataset][key]:
            if os.path.exists('../MAGIC-main/data_cl/{}/'.format(dataset) + file + '.txt'):
                continue
            f = open('../MAGIC-main/data/{}/'.format(dataset) + file, 'r', encoding='utf-8')
            fw = open('../MAGIC-main/data_cl/{}/'.format(dataset) + file + '.txt', 'w', encoding='utf-8')
...
def read_graphs(dataset):
    malicious_entities = '../MAGIC-main/data/{}/{}.txt'.format(dataset.split('_')[0], dataset.split('_')[0])
    # malicious_entities = '../MAGIC-main/data/{}/{}.txt'.format(dataset, dataset)
    f = open(malicious_entities, 'r')
    malicious_entities = set()
    for l in f.readlines():
        malicious_entities.add(l.lstrip().rstrip())

    if not os.path.exists('./data_cl/{}/types.json'.format(dataset)):
        if "optc" not in dataset:
            preprocess_dataset(dataset)
        else:
            preprocess_optc_dataset(dataset, metadata)
    train_gs = []
    for file in metadata[dataset]['train']:
        _, train_g, node_uuid2cnt = read_single_graph(dataset, malicious_entities, file, False)
        train_gs.append((train_g,node_uuid2cnt))
    test_gs = []
    test_node_map = {}
    count_node = 0
    for file in metadata[dataset]['test']:
        node_map, test_g, node_uuid2cnt = read_single_graph(dataset, malicious_entities, file, True)
        assert len(node_map) == test_g.number_of_nodes()
        test_gs.append((test_g, node_uuid2cnt))
        for key in node_map:
            if key not in test_node_map:
                test_node_map[key] = node_map[key] + count_node
        count_node += test_g.number_of_nodes()

    if os.path.exists('./data_cl/{}/names.json'.format(dataset)) and os.path.exists('./data_cl/{}/types.json'.format(dataset)):
        with open('./data_cl/{}/names.json'.format(dataset), 'r', encoding='utf-8') as f:
            id_nodename_map = json.load(f)
        with open('./data_cl/{}/types.json'.format(dataset), 'r', encoding='utf-8') as f:
            id_nodetype_map = json.load(f)
        f = open('./data_cl/{}/malicious_names.txt'.format(dataset), 'w', encoding='utf-8')
    ...
    # dump the edge_type_dict and node_type_dict into json format
    with open('./data_cl/{}/edge_type_dict.json'.format(dataset), 'w', encoding='utf-8') as f:
        json.dump(edge_type_dict, f)
    with open('./data_cl/{}/node_type_dict.json'.format(dataset), 'w', encoding='utf-8') as f:
        json.dump(node_type_dict, f)
    pkl.dump((final_malicious_entities, malicious_names), open('./data_cl/{}/malicious.pkl'.format(dataset), 'wb'))
    pkl.dump([(nx.node_link_data(train_g[0]), train_g[1]) for train_g in train_gs], open('./data_cl/{}/train.pkl'.format(dataset), 'wb'))
    pkl.dump([(nx.node_link_data(test_g[0]), test_g[1]) for test_g in test_gs], open('./data_cl/{}/test.pkl'.format(dataset), 'wb'))

    # 删除metadata.json
    os.remove('./data_cl/{}/metadata.json'.format(dataset))

if __name__ == '__main__':
    args = build_args()
    if args.dataset not in ['trace', 'theia', 'cadets', 'optc_1','optc_2','optc_3']:
